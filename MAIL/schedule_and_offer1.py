...
# ----------------- Demo commands -----------------
def demo_invite(candidates, topic="Data Analyst"):
    """
    Send interview invites to all candidates provided by previous function.
    candidates: list of dicts with keys {'name', 'email', 'phone'}
    """
    create_tables()
    ...
    for cand in candidates:
        ...
        if not candidate_email:
            print(f"⚠️ Skipping {candidate_name} (no email provided)")
            continue

        print(f"\n📩 Sending invite to {candidate_name} ({candidate_email}) for topic '{topic}'")
        try:
            schedule_interview_and_send_invite(
            ...
            print(f"❌ Failed to send invite to {candidate_email}: {e}")
